...
class PlayState extends MusicBeatState
{
			...
			if (isDuel) {
				for (sid => player in GameClient.room.state.players) {
					if (sid != GameClient.room.sessionId) {
						...
						setOnScripts('duoOpponentSID', sid);
					}
				}
			}
		}
	...
	override function destroy() {
		...
		// 	stage3D = null;
		// }
		CoolUtil.showPopUp("State's itself destroying", "Trace");
		super.destroy();
		CoolUtil.showPopUp("State destroyed", "Trace");
	}
